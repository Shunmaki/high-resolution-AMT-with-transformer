    ...
    for n, hdf5_path in enumerate(hdf5_paths):
        with h5py.File(hdf5_path, 'r') as hf:
            if hf.attrs['split'].decode() == split:
                ...
                # Get ground truths
                (target_dict, note_events, pedal_events) =                     target_processor.process(start_time=0, 
                ...
                if 'pedal_frame_output' in output_dict.keys():
                    total_dict['ref_pedal_on_off_pairs'] =                         np.array([[event['onset_time'], event['offset_time']] for event in pedal_events])
...
class ScoreCalculator(object):
    def __init__(self, hdf5s_dir, probs_dir, split, post_processor_type='regression'):
        ...
        self.velocity = False  # True | False  Note4での評価を行うか
        self.pedal = False

        self.evaluate_frame = True # True | False  frameレベルでの評価を行うか
        self.onset_tolerance = 0.05
        self.offset_ratio = None  # None | 0.2  Note3での評価を行うか
    ...
    def calculate_score_per_song(self, args):
        ...
        (est_on_off_note_vels, est_pedal_on_offs) =             post_processor.output_dict_to_note_pedal_arrays(output_dict)
